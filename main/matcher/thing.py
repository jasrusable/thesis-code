...
class Thing(object):
    ...
    def detect(self):
        self._test_keypoints, self._test_descriptors = (
            self._detector.detect(self._test_image.cv_image)
        )
        self._query_keypoints, self._query_descriptors = (
            self._detector.detect(self._train_image.cv_image)
        )

    def match(self):
        self._matches = self._matcher.match(
            self._test_descriptors, self._query_descriptors
        )
    ...
    def homogrophy(self):
        src_pts = np.float32([self._test_keypoints[match.queryIdx].pt
                            for match in self._matches]).reshape(-1, 1, 2)
        dst_pts = np.float32([self._query_keypoints[match.trainIdx].pt
                            for match in self._matches]).reshape(-1, 1, 2)
        ...
        scene_image = polylines(
            self._train_image.cv_image, [np.int32(dst)], True, 255, 3, LINE_AA
        )
        draw_params = dict(
            matchColor=(0, 255, 0),
